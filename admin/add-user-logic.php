...
if(isset($_POST['submit'])) {
    ...
    if (!$firstname) {
        ...
        if($createpassword != $confirmpassword) {
            $_SESSION['signup'] = "Passwords d not match";
        } else {
            // hash password
            ...
            if(mysqli_num_rows($user_check_result) > 0) {
                ...
                if(in_array($extention, $allowed_files)) {
                    //make sure image is not too large (50mb+)
                    if($avatar['size'] < 50000000) {
                        // upload avatar
                        move_uploaded_file($avatar_tmp_name, $avatar_destination_path);
                    } else {
                        $_SESSION['add-user'] = "File is too large. It should be less than 50mb";
                    }
                } else {
    ...
    // redirect back to signup page if there is ny problem
    ...
    // if button wan't clicked, bounce back to signup page
    header('location: ' . ROOT_URL . 'admin/add-user.php');
    die();
}