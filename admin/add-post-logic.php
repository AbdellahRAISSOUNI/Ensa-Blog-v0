...
if (isset($_POST['submit'])) {
    ...
    // set is_featured t 0 if unchecked
    ...
    if (!$title) {
        ...
        $_SESSION['add-pot'] = "Choose post thumbnail";
    } else {
        //work on thumbnail
        //rename the image
        ...
        if(in_array($extension, $allowed_files)) {
            // make sure image is not too big. (20mb+)
            if($thumbnail['size'] < 20_000_000) {
                // upload thumbnail
                move_uploaded_file($thumbnail_tmp_name, $thumbnail_destination_path);
            } else {
                $_SESSION['add-post'] = "File size too big. Should be less than 20mb";
            }
        } else {
    ...
    if(isset($_SESSION['add-post'])) {
        ...
        // set is_featud of all posts to 0 if is_featured for this post is 1
        if ($is_featured == 1) {
